  ...
  return (
    <div className={styles.panel}>
      <div className={styles.header}>
        <h3 className={styles.title}>{stepNumber}. {t("setup.participants.manage_title")} ({participants.length})</h3>
      </div>
      <p className={styles.helper}>
        {t("setup.participants.manage_desc")}
      </p>

      {participants.length === 0 ? (
        <p className={styles.empty}>{t("setup.participants.empty")}</p>
      ) : (
        <div className={styles.tableWrapper}>
          <table className={styles.table}>
            <thead>
              <tr>
                <th className={styles.colNum}>{t("setup.participants.table_num")}</th>
                <th className={styles.colName}>{t("setup.participants.table_name")}</th>
                <th className={styles.colActions}>{t("setup.participants.table_actions")}</th>
              </tr>
            </thead>
            <tbody>
              {participants.map((p, i) => (
                <tr key={p.id} className={styles.row}>
                  ...
                  <td className={styles.actions}>
                    {editing?.id === p.id ? (
                      <button className={styles.saveBtn} onClick={commitEdit} type="button" title={t("common.save")}>
                        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14">
                          <polyline points="2,8 6,13 14,3" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      </button>
                    ) : (
                      <button className={styles.editBtn} onClick={() => startEdit(p)} type="button" title={`${t("common.edit")} ${p.name}`}>
